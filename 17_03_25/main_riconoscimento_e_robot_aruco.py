...
calibration_file = '07_03_25/image_corners_real_coords.json'

# File contenente i parametri di calibrazione della fotocamera
camera_calibration_file = '07_03_25/calibration_data_aruco.json'
...
if not ret:
    ...
    #reference_binary_image = np.where(saturation > 0.6, 1, 0).astype(np.uint8)
    reference_binary_image = np.where(saturation < 0.55, 1, 0).astype(np.uint8)
    ...
    ax[3].imshow(result_mask, cmap='gray')

    combined_mask = torch.logical_and(torch.tensor(result_mask),fastsam_final_mask)
    ax[4].title.set_text("Grayscale output")
    ax[4].imshow(combined_mask, cmap='gray')
